...
@section('content')
    ...
    <h2>{{ __('onboarding.routine_title') }}</h2>
    <p>{{ __('onboarding.routine_description') }}</p>
...
<div class="regcontent">
    ...
    <form id="resetPassword" action="" method="post">
        ...
        <label>{{ __('onboarding.work_start_question') }}</label>
        <div class="">
            ...
            <x-global::selectable selected="" :id="'daySchedule-workStart-3'" :name="'daySchedule-workStart-button'" :value="''" :label="''" class="compact" onclick="jQuery(this).hide(); jQuery('#daySchedule-workStart').show()">
                <label for="" class="">
                    <i class="fa fa-clock"></i> {{ __('onboarding.choose_custom_time') }}
                </label>
            </x-global::selectable>
        ...
        <br />
        <label>{{ __('onboarding.lunch_question') }}</label>
        <div class="">
        ...
        <br />
        <label>{{ __('onboarding.work_end_question') }}</label>
...
{{--        </div>--}}
        ...
        <div class="tw-text-right">
            <a href="{{BASE_URL}}/auth/userInvite/{{$inviteId}}?step=3" class="btn btn-secondary" style="width:auto; margin-right:10px">{{ __('buttons.back') }}</a>
...
<script>
    ...
    function showTimeForm($id) {
        ...
        jQuery('.dayOfWeekInputs.tw-flex').each(function(index){
            ...
            if(index == 0) {
                jQuery(this).find('.applyBox').html("<a href='javascript:void(0);'>{{ __('label.apply_to_all') }}")
            }else{
