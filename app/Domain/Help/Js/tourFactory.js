leantime.tourFactory = (function () {
    ...
    var getTourDefinition = function(tourName) {
        ...
        return (tourDefinitions[tourName] || []).map(function(step) {
            const translationPrefix = "tour.cobound." + tourName + "." + step.id;
            const translatedTitle = leantime.i18n.__(translationPrefix + ".title");
            const translatedText = leantime.i18n.__(translationPrefix + ".text");

            return {
                ...step,
                title: translatedTitle || step.title,
                text: translatedText ? translatedText.replaceAll("%s", leantime.appUrl) : step.text
            };
        });
    };
