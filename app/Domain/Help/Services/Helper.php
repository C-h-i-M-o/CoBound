...
class Helper
{
    ...
    public function getOnboardingChecklist(int $userId): array|false
    {
        ...
        $checklist = [
            'step1' => [
                'completed' => true,
                'label' => __('onboarding.checklist.first_task'),
            ],
            'step2' => [
                'completed' => false,
                'label' => __('onboarding.checklist.dashboard_tour'),
            ],
            'step3' => [
                'completed' => false,
                'label' => __('onboarding.checklist.personal_project'),
                'url' => '',
            ],
            'step4' => [
                'completed' => false,
                'label' => __('onboarding.checklist.project'),
                'url' => '',
            ],
            'step5' => [
                'completed' => false,
                'label' => __('onboarding.checklist.milestone'),
                'url' => '',
            ],
            'step6' => [
                'completed' => false,
                'label' => __('onboarding.checklist.goal'),
                'url' => '',
            ],
            'step7' => [
                'completed' => false,
                'label' => __('onboarding.checklist.comment'),
                'url' => '',
            ],
    ...
    public function createDefaultProject(int $userId, string $role = 'editor')
    {
        ...
        $values = [
            'name' => __('default_project.name'),
            'details' => __('default_project.details'),
            'clientId' => 0,
            'hourBudget' => 0,
        ...
        $values = [
            'headline' => __('default_project.milestone'),
            'projectId' => $projectId,
            'editorId' => $userId,
            'userId' => $userId,
        ...
        $values['headline'] = __('default_project.task.community');
        $values['description'] = __('default_project.task.community_description');
        ...
        if (in_array($role, ['admin', 'owner', 'manager'])) {
            ...
            $values['headline'] = __('default_project.task.invite');
            $values['description'] = sprintf(__('default_project.task.invite_description'), BASE_URL);
            $values['dateToFinish'] = dtHelper()->userNow()->addDays(1)->formatDateForUser();
            $ticketService->quickAddTicket($values);
        }

        $values['headline'] = __('default_project.task.structure');
        $values['description'] = __('default_project.task.structure_description');
        $values['dateToFinish'] = dtHelper()->userNow()->addDays(1)->formatDateForUser();
        $ticketService->quickAddTicket($values);

        $values['headline'] = __('default_project.task.goal');
        $values['description'] = sprintf(__('default_project.task.goal_description'), BASE_URL);
        $values['dateToFinish'] = dtHelper()->userNow()->addDays(1)->formatDateForUser();
        $ticketService->quickAddTicket($values);

        $values['headline'] = __('default_project.task.milestone');
        $values['description'] = sprintf(__('default_project.task.milestone_description'), BASE_URL);
        $values['dateToFinish'] = dtHelper()->userNow()->addDays(1)->formatDateForUser();
        $ticketService->quickAddTicket($values);

        $values['headline'] = __('default_project.task.explore');
        $values['description'] = sprintf(__('default_project.task.explore_description'), BASE_URL, $projectId);
        $values['dateToFinish'] = dtHelper()->userNow()->addDays(1)->formatDateForUser();
        $ticketService->quickAddTicket($values);

        $values['headline'] = __('default_project.task.profile');
        $values['description'] = sprintf(__('default_project.task.profile_description'), BASE_URL);
        $values['dateToFinish'] = dtHelper()->userNow()->addDays(1)->formatDateForUser();
        $ticketService->quickAddTicket($values);

        $values['headline'] = __('default_project.task.first');
        $values['description'] = '';
        ...
        $values = [
            'title' => __('default_project.goal_canvas'),
            'author' => $userId,
            'projectId' => $projectId,
        ];
        ...
        $values = [
            'description' => __('default_project.goal_metric'), // Metric
            'title' => __('default_project.goal_title'), // Objective
            'box' => 'goal',
            'author' => $userId,
