...
class NewUser extends Controller
{
    ...
    public function post(array $params): Response
    {
        ...
        if (isset($_POST['save'])) {
            ...
            } elseif ($result === 'create_failed') {
                $this->tpl->setNotification($this->language->__('notification.user_invite_create_failed'), 'error');
            } else {
                $this->tpl->setNotification(
                    $this->language->__('notification.invite_link_created'),
                    'success',
                    'user_invited'
                );

                return Frontcontroller::redirect(BASE_URL.'/users/editUser/'.(int) $result.'?inviteCreated=1');
            }
        }
