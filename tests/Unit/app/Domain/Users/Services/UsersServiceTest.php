...
class UsersServiceTest extends TestCase
{
    ...
    public function test_create_user_invite_only_persists_invite_token_without_sending_email(): void
    {
        $savedValues = null;
        $repo = $this->make(UserRepository::class, [
            'addUser' => function (array $values) use (&$savedValues): string {
                $savedValues = $values;
        ...
        $result = $this->makeService($repo)->createUserInvite([
            'user' => 'invitee@example.com',
        ]);

        $this->assertSame(42, $result);
        $this->assertSame('i', $savedValues['status']);
        $this->assertNotEmpty($savedValues['pwReset']);
        $this->assertNotEmpty($savedValues['password']);
    }
